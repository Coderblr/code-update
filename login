...
export default function LoginPage({ onLoginSuccess, onNavigate }) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await fetch("http://10.1.161:5050/token", {
                method: "POST",
            ...
            // Pass the token up to the parent to fetch user profile
